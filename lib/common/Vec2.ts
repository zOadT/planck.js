...
import { EPSILON, isFinite, invSqrt } from './Math';
...
export default class Vec2 {
  ...
  // public static _deserialize(data: { x: f64, y: f64 }): Vec2 {
   ...
   * @returns this
   */
  public set<T>(x: T, y: f64 = NaN): this {
  ...
  public static areEqual(v: Vec2, w: Vec2 | null): boolean {
    // _ASSERT && Vec2.assert(v);
    // _ASSERT && Vec2.assert(w);
   ...
   */
  public static crossVec2(v: Vec2, w: Vec2): f64 {
   ...
   * vector.
   */
  public static cross<T, U>(v: T, w: U): Vec2 {
    if(isFloat(w) && v instanceof Vec2) {
      // _ASSERT && Vec2.assert(v);
      // _ASSERT && Math.assert(w);
      return Vec2.neo(w * v.y, -w * v.x);
    
    } else if(w instanceof Vec2 && isFloat(v)) {
   ...
   * Returns `a + (v x w)`
   */
  public static addCross<T, U>(a: Vec2, v: T, w: U): Vec2 {
    if (v instanceof Vec2 && isFloat(w)) {
      // _ASSERT && Vec2.assert(v);
      // _ASSERT && Math.assert(w);
      return Vec2.neo(w * v.y + a.x, -w * v.x + a.y);
  
    } else if (isFloat(v) && w instanceof Vec2) {
      // _ASSERT && Math.assert(v);
      // _ASSERT && Vec2.assert(w);
  ...
  public static mul<T, U>(a: T, b: U): Vec2 {
    if(a instanceof Vec2 && isFloat(b)) {
      // _ASSERT && Vec2.assert(a);
      // _ASSERT && Math.assert(b);
      return Vec2.neo(a.x * b, a.y * b);
    }
    if(isFloat(a) && b instanceof Vec2) {
      // _ASSERT && Math.assert(a);
